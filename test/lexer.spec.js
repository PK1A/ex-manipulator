...
describe('lexer', function () {
    ...
    describe('empty input', function () {
        it('should return an empty array of tokens for empty input', function () {
            expect(lexer()).to.deep.equal([]);
            expect(lexer('')).to.deep.equal([]);
            expect(lexer(null)).to.deep.equal([]);
        });
    });
    ...
    describe('numbers', function () {
        ...
        it('should recognise integers', function () {
            expect(lexer('0')).to.deep.equal([{t: 'num', v: 0, f: 0}]);
            expect(lexer('10')).to.deep.equal([{t: 'num', v: 10, f: 0}]);
            expect(lexer('100')).to.deep.equal([{t: 'num', v: 100, f: 0}]);
            expect(lexer('1000')).to.deep.equal([{t: 'num', v: 1000, f: 0}]);
            expect(lexer('12345')).to.deep.equal([{t: 'num', v: 12345, f: 0}]);
        });

        it('should recognise floats', function () {
            expect(lexer('0.0')).to.deep.equal([{t: 'num', v: 0.0, f: 0}]);
            expect(lexer('12.345')).to.deep.equal([{t: 'num', v: 12.345, f: 0}]);
        });
    });
    ...
    describe('identifiers (names)', function () {
        ...
        it('should properly recognise names', function () {
            expect(lexer('f')).to.deep.equal([{t: 'idn', v: 'f', f: 0}]);
            expect(lexer('foo')).to.deep.equal([{t: 'idn', v: 'foo', f: 0}]);
            expect(lexer('$foo')).to.deep.equal([{t: 'idn', v: '$foo', f: 0}]);
            expect(lexer('$$foo')).to.deep.equal([{t: 'idn', v: '$$foo', f: 0}]);
            expect(lexer('_foo')).to.deep.equal([{t: 'idn', v: '_foo', f: 0}]);
            expect(lexer('foo10')).to.deep.equal([{t: 'idn', v: 'foo10', f: 0}]);
        });
    });
    ...
    describe('strings', function () {
        ...
        it('should recognise single quoted strings', function () {
            expect(lexer("''")).to.deep.equal([{t: 'str', v: '', f: 0}]);
            expect(lexer("'foo'")).to.deep.equal([{t: 'str', v: 'foo', f: 0}]);
            expect(lexer(" 'foo bar' ")).to.deep.equal([{t: 'str', v: 'foo bar', f: 1}]);
        });
        ...
        it('should throw on unterminated string', function () {
            expect(function(){
                lexer('"')
            }).to.throw('Error parsing """: unfinished string at 0');
        });

        it('should throw on unterminated string', function () {
            expect(function(){
                lexer('"fooo" " ooo')
            }).to.throw('Error parsing ""fooo" " ooo": unfinished string at 7');
        });
    ...
    describe('operators', function () {
        ...
        describe('single-char operators', function () {
            ...
            it('it should return operator token for single-char operatorss', function () {
                expect(lexer('+')).to.deep.equal([{t: 'opr', v: '+', f: 0}]);
                expect(lexer('-')).to.deep.equal([{t: 'opr', v: '-', f: 0}]);
                expect(lexer('*')).to.deep.equal([{t: 'opr', v: '*', f: 0}]);
                expect(lexer('/')).to.deep.equal([{t: 'opr', v: '/', f: 0}]);
                expect(lexer('%')).to.deep.equal([{t: 'opr', v: '%', f: 0}]);
    ...
    describe('expressions', function () {
        ...
        it('should tokenize integers separates by whitespaces', function () {
            expect(lexer(' 12 34 5')).to.deep.equal([
                {t: 'num', v: 12, f: 1},
                {t: 'num', v: 34, f: 4},
                {t: 'num', v: 5, f: 7}
            ]);
        ...
        it('should tokenize', function() {
            expect(lexer('"foo".toUpperCase()')).to.deep.equal([
                {t: 'str', v: 'foo', f: 0},
                {t: 'opr', v: '.', f: 5},
